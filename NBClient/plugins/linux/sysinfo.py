    ...
    for line in raw_data:
        if next_ip_line:
            ...
            if mac_addr not in nic_dic:
                nic_dic[mac_addr] = {'name': nic_name, 'macaddress': mac_addr, 'netmask': netmask, 'network': network,
                                     'bonding': 0, 'model': 'unknown', 'ipaddress': ip_addr, }
                ...
                nic_dic[random_mac_addr] = {'name': nic_name, 'macaddress': random_mac_addr, 'netmask': netmask,
                                            'network': network, 'bonding': 1, 'model': 'unknown',
                                            'ipaddress': ip_addr, }
...
def raminfo():
    ...
    raw_total_size = str(raw_total_size)
...
def osinfo():
    ...
    data_dic = {"os_distribution": distributor[1].strip() if len(distributor) > 1 else None,
                "os_release": release[1].strip() if len(release) > 1 else None, "os_type": "linux", }
    # print(data_dic)
    return data_dic
...
def cpuinfo():
    ...
    raw_data = {'cpu_model': "%s |grep 'model name' |head -1 " % base_cmd,
    ...
    data = {"cpu_count": raw_data["cpu_count"], "cpu_core_count": raw_data["cpu_core_count"]}
