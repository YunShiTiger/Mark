...
class Asset(object):
    ...
    def __update_asset_component(self, data_source, fk, update_fields, identify_field=None):
        ...
        identify_field: use this field to identify each component of an Asset , if set to None,
        means only use asset id to identify
         """
                ...
                for obj in objects_from_db:
                    ...
                    if type(data_source) is list:
                        for source_data_item in data_source:
                            # 到客户端的数据 源里去找到跟 服务器数据库中key_field_data对应的条目
                            ...
                            print(
                                ...
                                'component data is missing in reporting data!\033[0m' % (
                                    key_field_data))
                            self.response_msg(
                        ...
                        for key, source_data_item in data_source.items():
                            ...
                            print(
                                '\033[33;1mWarning:cannot find any matches in source data by using key field val [%s],'
                                'component data is missing in reporting data!\033[0m' % (
                                    key_field_data))
                    else:
